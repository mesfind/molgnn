...
class AqSolDB(InMemoryDataset):
  ...
  """
    ...
    def __init__(self, root_dir: str, name, smi_idx, target_idx,
        ...
        #skip calling data
        super(AqSolDB, self).__init__(root_dir, transform, pre_transform, pre_filter)
    ...
    def mol_desc(self,smiles):
        '''
        Return list of descriptors for each smiles string
        '''
        # descr_list = ['TPSA', 'MolWt'] # list of descriptors required for aqeous solubility
        # descr = []
        # for des in descr_list:
        #     m = Chem.MolFromSmiles(des[0])
        #     calc = MoleculeDescriptors.MolecularDescriptorCalculator(des)
        #     each_mol = [ i for i in calc.CalcDescriptors(m)]
        #     each_mol.append(des[0])
        #     descr.append(each_mol)
        #     x = np.array(descr)
        #     descr_norm = (x[0]-min(x[0]))/(max(x[0])-min(x[0]))
        # return descr
    ...
    def process(self): 
        ...
        for line in tqdm(dataset):
            ...
            # descriptors for solublity
            descr_list = ['TPSA','NumAromaticCarbocycles','NumSaturatedCarbocycles', 'NumHDonors','NumHAcceptors','MolLogP','NumRadicalElectrons','NumRotatableBonds'] #list of descriptors required for aqeous solubility
            calc = MoleculeDescriptors.MolecularDescriptorCalculator(descr_list)
            descr_array = np.array(list(calc.CalcDescriptors(mol)))
            descr_norm = (descr_array-min(descr_array))/(max(descr_array)-min(descr_array))
            descr = descr_norm.tolist()
            ...
            for atom in mol.GetAtoms():
                ...
                x1 = torch.tensor([atomic_num_scaled]+ valance + [formal_charge] +
            ...
            for bond in mol.GetBonds():
                ...
                #edge_attr = torch.tensor([single, double, triple, aromatic, conjugation, ring] + stereo+descr)
...
class AqSolDB2(InMemoryDataset):
    r"""
  AqSolDB is a computational database that covers a focused chemical space of two
  classes of organic molecules (quinones and aza-aromatics) that have found to be
  highly promising for aqueous redox flow batteries. RedDB's data is generated using
  simulation tools that apply cheminformatics, machine learning, molecular mechanics,
  and quantum chemistry methods. RedDB contains structural information and several
  ...
  Args:
      root (string): Root directory where the dataset should be saved.
      name (string): The name of the dataset (:obj:`"AqSolDB"`).
      transform (callable, optional): A function/transform that takes in an
          :obj:`torch_geometric.data.Data` object and returns a transformed
          version. The data object will be transformed before every access.
          (default: :obj:`None`)
  ...
  """
    ...
    def __init__(self, root_dir: str, name, smi_idx, target_idx,
        ...
        self.target_idx = target_idx
        #skip calling data
        super(AqSolDB2, self).__init__(root_dir, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
        ...
        for line in tqdm(dataset):
            ...
            for atom in mol.GetAtoms():
                ...
                x1 = torch.tensor([atomic_num_scaled]+ valance + [formal_charge] +
                             [radical_electrons] + hybridization +
                             [aromaticity] + hydrogens+  [chirality] +
                             chirality_type + [atomic_mass] +[vdw_radius]+[covalent_radius]+descr, dtype=torch.float)
                xs.append(x1)
                x = torch.stack(xs, dim=0)
