...
class GenMolGraph(InMemoryDataset):
    ...
    def process(self):
        ...
        for line in tqdm(dataset):
            ...
            descr_list = ['TPSA','NumAromaticCarbocycles', 'NumHDonors','NumHAcceptors','MolLogP','NumRadicalElectrons','NumRotatableBonds']
            #list of descriptors required for aqeous solubility
            ...
            #AllChem.EmbedMultipleConfs(mol, 1, numThreads=0,useRandomCoords=True) #can allow user to change numConfs
...
class GenMolFeatures(object):
    ...
    def __call__(self, data):
        ...
        for atom in mol.GetAtoms():
            ...
            x = torch.tensor([atomic_num_scaled]+ valance + [formal_charge] +
                             [radical_electrons] + hybridization +
                             [aromaticity] + hydrogens + [chirality] +
                             chirality_type+[atomic_mass] + [vdw_radius]+ [covalent_radius],dtype=torch.float)
            xs.append(x)
